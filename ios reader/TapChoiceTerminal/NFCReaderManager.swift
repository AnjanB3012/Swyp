...
private enum NFCExchangeError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .status(sw1, sw2):
            (sw1 == 0x69 && sw2 == 0x85)
                ? "Open Pay in Swyp on Android, select a card, tap Ready to pay, then try again."
                : String(format: "Swyp returned status %02X%02X", sw1, sw2)
...
/// Reads only the private Swyp AID. No payment-network APDUs are used.
final class NFCReaderManager: NSObject, ObservableObject, NFCTagReaderSessionDelegate {
    ...
    @Published private(set) var approvedCredential: SwypCredential?

    private var session: NFCTagReaderSession?
    private var amountCents: Int64 = 100
    private var merchant = ""
    private var consumedNonces = Set<String>()
    private let terminalId = "swyp-terminal-01"

    var isScanning: Bool {
        [.waiting, .reading, .authorizing].contains(phase)
    }

    func begin(merchant: String, amountCents: Int64) {
    ...
    private func selectApplication(on tag: NFCISO7816Tag, session: NFCTagReaderSession) {
        guard let aid = Data(hex: SwypProtocol.aidHex) else {
    ...
    private func readCredential(on tag: NFCISO7816Tag, session: NFCTagReaderSession) {
        let apdu = NFCISO7816APDU(
            instructionClass: 0x80, instructionCode: SwypProtocol.getCredentialINS,
        ...
        send(apdu, to: tag) { [weak self] result in
            ...
            do {
                ...
                let credential = try JSONDecoder().decode(SwypCredential.self, from: data)
    ...
    private func authorize(
        ...
        do {
            ...
            let apdu = NFCISO7816APDU(
                instructionClass: 0x80, instructionCode: SwypProtocol.authorizeINS,
                p1Parameter: 0x00, p2Parameter: 0x00,
                ...
                do {
                    ...
                    BackendClient.authorizeIfConfigured(
                        ...
                        case .success:
                            ...
                            session.alertMessage = "Payment approved — separate the devices."
                            DispatchQueue.main.async {
    ...
    private func send(
        ...
        tag.sendCommand(apdu: apdu) { data, sw1, sw2, error in
            ...
            guard (UInt16(sw1) << 8 | UInt16(sw2)) == SwypProtocol.success else {
